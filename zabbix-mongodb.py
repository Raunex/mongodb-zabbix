...
class MongoDB(object):
    ...
    def get_db_system_namespaces(self):
        """get DB system namespaces"""
        if self.__conn is None:
            self.connect()
        if self.__dbnames is None:
            self.get_db_names()
        if self.__dbnames is not None:
            for mongo_db in self.__dbnames:
                db_handler = self.__conn[mongo_db]
                dbs = db_handler.command('db.system.namespaces.count')
                for k, v in dbs.items():
                    if k in ['namespaces.count']:
                        self.add_metrics('mongodb.namespaces.' + k +
                                         '[' + mongo_db + ']', int(v))
